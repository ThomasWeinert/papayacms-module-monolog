...
namespace Papaya\Module\Monolog\Logger\Profile {
  ...
  class SyslogUdpHandler implements Logger\Profile {
    ...
    public function createOptionsEditor(Logger\Options $options) {
      ...
      $dialog->fields[] = new UI\Dialog\Field\Input(
        new UI\Text\Translated('Host'), 'host', 100, self::$_DEFAULTS['host']
      );
      $dialog->fields[] = new UI\Dialog\Field\Input\Number(
        new UI\Text\Translated('Port'), 'port', self::$_DEFAULTS['port']
      );
      ...
      $dialog->fields[] = new UI\Dialog\Field\Select(
        new UI\Text\Translated('Facility'), 'facility', self::$_FACILITIES, FALSE
      );
      return $editor;
    }
  }
